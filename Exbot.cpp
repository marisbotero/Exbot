...
MotorDC MI,MD;
int VI=210,VD=200;
int PwmMI= 5;   //Puerto de Pwm
int DirMI= 4;   //Puerto digital
int PwmMD= 6;   //Puerto de Pwm
int DirMD= 7;   //Puerto digital
...
void SaveIRValues(void){
	if(digitalRead(CONFIGPIN)==HIGH || Activado==1){
		delay(200);
		Activado=1;
		//Serial.println("HIGH");
		if(ConfKey(EstadoConf)==1){
			...
			Serial.println("");
			Serial.print("Oprima tecla posicion ");
			Serial.println(EstadoConf);
		}
		else{
			if(EstadoConf==0 && flag==0){
				T1=millis();
				flag=1;
				Serial.print("Oprima tecla posicion ");
				Serial.println(EstadoConf);
...
void SeguirLinea(int UmbralIzq,int UmbralDer, int VelI,int VelD){
	//digitalWrite(pinIR,HIGH);
	VI=VelI;
	VD=VelD;
	int SensorI,SensorD,SensorC;
	...
	//Accion y Decision
	/*if(SensorC<Umbral){//Adelante
		Atras(VI,VD,1);}
	else */if(SensorI<UmbralIzq && SensorD<UmbralDer){//Adelante
		Adelante(VI,VD,1);
	}else if(SensorI>UmbralIzq){//Izquierda
		Derecha(VI,VD,1);
	}else if(SensorD>UmbralDer){//Derecha
		Izquierda(VI,VD,1);
...
void Adelante(int PwmIzq,int PwmDer, int Time)
{
  digitalWrite(DirMI, LOW);
  digitalWrite(DirMD, LOW);
  analogWrite(PwmMI,PwmIzq);
  analogWrite(PwmMD,PwmDer);
  delay(Time);
  
}
void Atras(int PwmIzq,int PwmDer, int Time) 
{
  digitalWrite(DirMI, HIGH);
  digitalWrite(DirMD, HIGH);
  analogWrite(PwmMI,PwmIzq);
  analogWrite(PwmMD,PwmDer);
  delay(Time);
}
void Derecha(int PwmIzq,int PwmDer, int Time) 
{
  digitalWrite(DirMD, HIGH);
  digitalWrite(DirMI, LOW);
  analogWrite(PwmMI,PwmIzq);
...
void Izquierda(int PwmIzq,int PwmDer, int Time) 
{
  digitalWrite(DirMD, LOW);
  digitalWrite(DirMI, HIGH);
  analogWrite(PwmMI,PwmIzq);
...
void SeguidorDeLuz(int UmbralIzq, int UmbralDer, int VelI, int VelD){
	...
	//Accion y Decision
	if(SensorI<UmbralIzq && SensorD<UmbralDer){//Adelante
		Adelante(VI,VD,1);
	}else if(SensorI>UmbralIzq && SensorD<UmbralDer){//Izquierda
		Izquierda(VI,VD,1);	
	}else if(SensorI<UmbralIzq && SensorD>UmbralDer){//Derecha	
		Derecha(VI,VD,1);
	}else if(SensorI>UmbralIzq && SensorD>UmbralDer){ //Parar
		Parar(1);
	}
	
}
void Iniciar_Motores(int PinVelMI, int PinDirMI,int PinVelMD, int PinDirMD)
{
	pinMode(PinVelMI,OUTPUT);
	pinMode(PinDirMI,OUTPUT);
	pinMode(PinVelMD,OUTPUT);
	pinMode(PinDirMD,OUTPUT);
}
